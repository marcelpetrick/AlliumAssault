...
import { dryScatter, drownedBy, gridSurface } from './geometry';
import type { Theme } from './themes';

/** How far a scattered tree is sunk into the hill so its trunk does not float above the surface. */
const TREE_SINK = 0.3;
/** Dry ground a tree needs under it: enough that it reads as standing on land, not paddling in it. */
const TREE_CLEARANCE = 1.2;
...
export class Environment {
  ...
  /** Scattered trees per hill band, so the rising sea can take them back. */
  private readonly groves: { mesh: Mesh; bases: number[]; matrices: Matrix[]; sunk: boolean[] }[] = [];
  /** Sea level the groves were last trimmed to; they only change when it moves. */
  private groveWaterLevel = Number.NaN;

  /**
   * Sink the background trees as Sudden Death raises the sea.
   *
   * The hills are built once, with the water where the match started, so without this the flood
   * simply climbs past them and leaves trees standing in open water — which is exactly what it
   * looks like. A swallowed tree is scaled away rather than moved, so nothing has to be rebuilt.
   */
  private trimGroves(waterLevel: number): void {
    if (waterLevel === this.groveWaterLevel) return;
    this.groveWaterLevel = waterLevel;
    const gone = Matrix.Scaling(0, 0, 0);
    for (const grove of this.groves) {
      const drowned = drownedBy(grove.bases, waterLevel);
      let moved = false;
      for (let n = 0; n < drowned.length; n++) {
        if (drowned[n] === grove.sunk[n]) continue;
        grove.sunk[n] = drowned[n];
        grove.mesh.thinInstanceSetMatrixAt(n, drowned[n] ? gone : grove.matrices[n], false);
        moved = true;
      }
      if (moved) grove.mesh.thinInstanceBufferUpdated('matrix');
    }
  }
  ...
  private buildHills(worldWidth: number, waterLevel: number): void {
    ...
    layers.forEach((layer, index) => {
      ...
      if (index < 2) {
        ...
        inst.isVisible = true;
        // The hill is drawn as rows of triangles between the sampled points, so this — not the
        // smooth function behind it — is the ground a tree stands on and the ground to measure
        // against the waterline.
        const surface = (at: number, t: number) => gridSurface(xs, zs, height, at, t);
        const buffer: number[] = [];
        ...
        for (let n = 0; n < count; n++) {
          ...
          const spin = rng() * 6.28;
          // A band fades to just under the waterline at its front and back edge, so a spot is only
          // usable where there is dry ground to stand on — otherwise a tree ends up in the sea.
          const t = dryScatter((at) => surface(x, at), waterLevel, TREE_CLEARANCE, [0.2 + rng() * 0.45, 0.35 + rng() * 0.3, 0.5]);
          if (t === null) continue;
          const base = surface(x, t) - TREE_SINK;
          const matrix = Matrix.Compose(
            new Vector3(s, stretch, s),
            Quaternion.RotationAxis(Vector3.Up(), spin),
            new Vector3(x, base, layer.z + t * layer.depth),
          );
          matrix.copyToArray(one, 0);
          buffer.push(...one);
          bases.push(base);
          matrices.push(matrix);
        }
        // Not a static buffer: the sea rises during Sudden Death and takes these back.
        inst.thinInstanceSetBuffer('matrix', new Float32Array(buffer), 16, false);
        this.groves.push({ mesh: inst, bases, matrices, sunk: bases.map(() => false) });
