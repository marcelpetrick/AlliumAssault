...
| T43  | Linters following best practices, findings fixed, part of the pipeline on every push                                                                    | See 6.3; everything runs through `npm run lint`, which `npm run verify` and CI already run on every push and pull request                                                                                                 | ✅ 1.26.0                                                                 |
| T44  | New `/reviewBranch` of the full code, fix the worst findings                                                                                            | Review the whole codebase into `review.md`, fix the highest-impact findings, re-verify                                                                                                                                    | ✅ 1.26.14                                                                |
...
| T12b | Review the batch and test                                                                                                                               | `/reviewBranch`-style review of the batch into `review.md`, fix findings, full `npm run verify`                                                                                                                           | ✅ 1.26.14                                                                |
| T13b | Publish                                                                                                                                                 | Push, release tag, Pages deployment, confirm live                                                                                                                                                                         | ✅ v1.26.16                                                               |
...
Outcome (1.26.0): ESLint now runs typescript-eslint `strictTypeChecked` + `stylisticTypeChecked`;
all 298 findings were fixed (invariant helpers `defined()` and `query()` instead of non-null
assertions, typed audio voices) except two deliberate config choices: numbers are allowed in
template strings, and non-null assertions are allowed in test code. Stylelint turned off
`no-descending-specificity` (it compares unrelated components). markdownlint turned off line
length and inline HTML (badges, centred screenshots). A dev-only vulnerability pulled in by
markdownlint-cli2 (smol-toml 1.7.0) is overridden to 1.8.0; `npm audit` reports none.
